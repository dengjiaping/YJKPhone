...
public class RoundProgressBar extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		gradient = new SweepGradient(getWidth() / 2, getHeight() / 2,
				new int[] { Color.parseColor("#44DB32"), Color.parseColor("#06B0EE"), Color.parseColor("#06B0EE"), Color.parseColor("#D30F5E"),
						Color.parseColor("#44DB32"),Color.parseColor("#44DB32")}, null);
		...
		float text_one_width = paint.measureText(text_one+" ");
		float text_two_width = paint.measureText(text_three+" ");

		if (textIsDisplayable && percent != 0 && style == STROKE) {
			canvas.drawText(text_one, centre - text_one_width / 2, centre + textSize
					/ 2 - 80, paint); // 画出进度百分比
			canvas.drawText(text_three, centre - text_two_width / 2, centre
					+ textSize / 2 + 80, paint); // 画出进度百分比
	 ...
	 * @param progress
	 */
	public synchronized void setProgress(int progress, String s1, String s2) {
		if (progress < 0) {
