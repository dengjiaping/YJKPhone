...
public class BraceletSportActivity extends BaseActivity implements OnClickListener {
	private TextView tvIncludeTitle;
	private Button btnBack;
	private CircleProgressBar circleProgressBar;// 自定义圆环进度条
	private TextView tvTitle;
	private TextView tvStepNum, tvSportTime, tvCalorie, tvNum,tvHeart;// 步数 运动时长 卡路里 心率
	// 超过百分比
	private TextView tvGoal;// 目标步数
	private TextView tvTotalNum;// 步数
		...
		public void handleMessage(final Message msg) {
			...
			switch (msg.what) {
				...
				case 3:
					...
					if(!BraceletSportActivity.this.isFinishing()){
...
//						FRToast.showToast(getApplicationContext(),"睡眠==" +sleepBean.toString());
						...
						sportInfo = new BraceletSportInfo(goalStepNum,
								sportBean.getSteps(), (int) sportBean.getCalories(),
								10, 10,0,sportBean.getDistance());
//						FRToast.showToast(getApplicationContext(),"运动==" +sportBean.toString());
						...
						sportInfo.setHeartRate(heartBean.getHeart_rate());
						PF = OK;
						initGetInfoResult();
						initReguest();
					}
	...
	private BraceletSportActivity mActivity;
	...
	protected void setUpViewAndData() {
		...
		System.out.println("memberId=" + memberId + "memberName=" + memberName
				+ "deviceName=" + deviceName + "accessToken=" + accessToken+"appCode="+appCode);
		init();
		if (HttpManager.isNetworkAvailable(this)) {
			GetInfoRequest re = new GetInfoRequest();
			re.execute(HttpManager.urlGetSportData(memberId, putDate));
...
//			}
	...
	private void init() {
		...
		tvIncludeTitle.setText(getResources()
				.getString(R.string.inputinfo_tv33));
		btnBack = (Button) findViewById(R.id.titlebar_btn_back);
		btnBack.setVisibility(View.VISIBLE);
		btnBack.setOnClickListener(this);
		tvTitle = (TextView) findViewById(R.id.brasport_tv_title);
		circleProgressBar = (CircleProgressBar) findViewById(R.id.brasport_progressbar);
		tvStepNum = (TextView) findViewById(R.id.brasport_tv_bushu);
		tvSportTime = (TextView) findViewById(R.id.brasport_tv_shichang);
		tvCalorie = (TextView) findViewById(R.id.brasport_tv_kaluli);
		tvHeart = (TextView) findViewById(R.id.brasport_tv_xinlv);
		tvTotalNum = (TextView) findViewById(R.id.brasport_tv_total);
		tvGoal = (TextView) findViewById(R.id.brasport_tv_goal);
		tvNum = (TextView) findViewById(R.id.brasport_tv_value);
		tvSync = (ImageView) findViewById(R.id.brasport_tv_sync);
		tvSync.setOnClickListener(this);
		tvDate = (TextView) findViewById(R.id.brasport_tv_date);
		btnLeft = (Button) findViewById(R.id.brasport_btn_left);
		btnRight = (Button) findViewById(R.id.brasport_btn_right);
		tvStatus = (TextView) findViewById(R.id.brasport_tv_title_devce_status);
...
//		initData();
	...
	private void initTitie() {
		ChangeDeviceInfo changeDeviceInfo = spInstance.getObj(memberId+SP_BLOOD_SELECT_YUNDONG, null);
...
//		}
	...
	private void initData() {
		goalStepNum = sportInfo.getGoalStepNum();
		if (sportInfo.getGoalStepNum() == 0) {
			sportInfo.setGoalStepNum(10000);
		}
		circleProgressBar.setMax(sportInfo.getGoalStepNum());
		circleProgressBar.setProgress(sportInfo.getStepNum(), 700);
		// 展示2h23'
		tvTotalNum.setText(sportInfo.getStepNum() + "");
		tvGoal.setText("目标" + sportInfo.getGoalStepNum() + "步");
		tvSportTime
				.setText(Constant.getHourFromS(sportInfo.getSportTime()));
		tvCalorie.setText(sportInfo.getCalorie() + "");
		tvStepNum.setText(sportInfo.getStepNum() + "");
		tvNum.setText(sportInfo.getNum() + "");
		tvHeart.setText(sportInfo.getHeartRate() + "");

	}

	public void onResume() {
		super.onResume();
		MobclickAgent.onPageStart("测量——运动页"); // 统计页面
		MobclickAgent.onResume(this); // 统计时长
	}

	public void onPause() {
		super.onPause();
		MobclickAgent.onPageEnd("测量——运动页"); // 保证 onPageEnd 在onPause 之前调用,因为
		// onPause 中会保存信息
	...
	public void onClick(View v) {
		switch (v.getId()) {
			...
			case R.id.titlebar_btn_change://选择设备
				// 选择默认设备
				init();
				Intent intent4 = new Intent(BraceletSportActivity.this,ChangeDeviceListActivity.class);
				intent4.setFlags(device_yundong);
...
//		}.getType());
	...
	private void initReguest() {
		...
		stringStringHashMap.put("activityCalories",""+sportInfo.getCalorie());
		stringStringHashMap.put("activitySteps",""+sportInfo.getStepNum());
		stringStringHashMap.put("activityMinutes",""+sportInfo.getSportTime());
		stringStringHashMap.put("heartRate",""+sportInfo.getHeartRate());
		stringStringHashMap.put("activityMeters",""+sportInfo.getDistance()/1000);
		stringStringHashMap.put("familyMemeberId",memberId);
		upLoadSleepRequest.execute(HttpManager.urlShouHuanData2(Constant.putSheBei("运动")),stringStringHashMap);
	}
	private class UpLoadSleepRequest extends AsyncTask<Object,Void,String>{
		...
		protected void onPreExecute() {
			progressDialog = new ProgressDialog(BraceletSportActivity.this);
			...
			if (result.toString().trim().equals("ERROR")) {
				...
				try {
					...
					if (resultCode.equals("1")) {
						...
						int num = jo2.optInt("activityPer");
						int activitySteps = jo2.optInt("activitySteps");
						int activityCalories = jo2.optInt("activityCalories");
						int activityMinutes = jo2.optInt("activityMinutes");
						int goalSteps = jo2.optInt("goalSteps");
						int heartRate = jo2.optInt("heartRate");
						double distance = jo2.optDouble("distance");
						sportInfo = new BraceletSportInfo(goalSteps,
								activitySteps, activityCalories,
								activityMinutes, num,heartRate,distance);
						PF = OK;
						initGetInfoResult();
	...
	private class SyncRequest extends AsyncTask<Object, Void, String> {
		...
		@Override
		protected void onPreExecute() {
			progressDialog2 = new ProgressDialog(BraceletSportActivity.this);
