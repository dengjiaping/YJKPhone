	...
	public void onClick(View v) {
		switch (v.getId()) {
			...
			case R.id.titlebar_btn_save:
				...
				if(sign.length()>30){
					Toast.makeText(PersonalInfoActivity.this, "个性签名不能超过30字！", Toast.LENGTH_SHORT).show();
					return;
				}
				//String str = tvSign.getText().toString().trim();
				nameStr = tvName.getText().toString().trim();
				CommitRequest re = new CommitRequest();
				re.execute(HttpManager.urlSaveInfo);
				break;
	...
	private class CommitRequest extends AsyncTask<Object, Void, String> {
		...
		protected String doInBackground(Object... params) {
			...
			if (!picPath.equals("")) {
				...
				result = HttpManager.postImage(url, map, files);
			} else {
				// 不更换头像
				result = HttpManager.getStringContent(HttpManager.urlSaveInfo2(
						userId, Constant.provinceId, Constant.cityId,
						Constant.districtId,nameStr, sign));
			}
			// Log.e("result", result + "");
			return result;
		}
