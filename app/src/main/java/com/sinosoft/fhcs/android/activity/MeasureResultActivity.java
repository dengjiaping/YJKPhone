...
public class MeasureResultActivity extends BaseActivity implements OnClickListener {
...
//					BloodPressureBean bloodPressureBean = JsonUtils.fromJson(msg.obj.toString(), BloodPressureBean.class);
					if(slimmingBean != null) {
						...
						double num = Double.parseDouble(String.valueOf(slimmingBean.getWeight()));
						ShebeiMeaInfo shebei=new ShebeiMeaInfo(measureTime, Double.valueOf(num), 0, 0, 0, 0, 0, 0, 0, 0, 0);
						shebeiResult(shebei);
						initReguest(shebei);
	...
	private void initStartData() {
		ChangeDeviceInfo changeDeviceInfo = spInstance.getObj(SP_BLOOD_SELECT_TIZHONG,null);
		...
		if (strTitle.equals("体重")) {
			...
			if(changeDeviceInfo != null){
				tvStateMsg.setText("请站上"+changeDeviceInfo.getDeviceName()+"体重秤");
			}else {
				tvStateMsg.setText("");
			}
			tvState.setTextColor(getResources().getColor(R.color.measure_red));
			...
			llMearsultStart.setVisibility(View.GONE);
	...
	public void onResume() {
		super.onResume();
		MobclickAgent.onPageStart("体重测量结果页"); // 统计页面
	...
	public void onPause() {
		super.onPause();
		MobclickAgent.onPageEnd("体重测量结果页"); // 保证 onPageEnd 在onPause 之前调用,因为
...
//						connectAndMeasure();
			...
			/*if(strTitle.equals("体重")){
				...
				bManager.connect(cId, MeasureResultActivity.this, user, null);
			}else if(strTitle.equals("血糖")){
				btnStartMeasure.setEnabled(false);

			}else{
				Toast.makeText(this, "暂时无法测量！", Toast.LENGTH_SHORT).show();
			}*/
				break;
			...
			case R.id.mearesult_ready_btnhelp:
				// 设备帮助
				if(strTitle.equals("体重")){
					Intent intent=new Intent(MeasureResultActivity.this,SheBeiHelpActivity.class);
					intent.putExtra("strTitle", strTitle);
					startActivity(intent);
				}else{
			...
			case R.id.mearesult_device_btn:
				...
				//sdkApi.autoConnectMeasure(SupportDevices.BLOOD_SUGAR_SANNUO, null, BloodSugerCallback);
				break;
			case R.id.mearesult_nodevice_btn:
				//没有设备时点击手动录入
				Intent intent2 = new Intent(MeasureResultActivity.this,
			...
			case R.id.mearesult_ready_btn:
				//有设备时点击手动录入
				Intent intent3 = new Intent(MeasureResultActivity.this,
						InputInfoActivity.class);
			...
			case R.id.titlebar_btn_change:
				//设备切换
				initStartData();
				Intent intent4 = new Intent(MeasureResultActivity.this,ChangeDeviceListActivity.class);
				intent4.putExtra("changeDevice", "blood");
				intent4.setFlags(device_tizhong);
